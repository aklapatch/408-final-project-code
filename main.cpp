...
int main()
{
    ...
    if (Specs.DatabaseTableName == "" || Specs.DatabaseTableName == " "){
        printf("\r\n No Database Table Name Specified, Exiting\r\n");
        return 1;
    }
    
    setESPMode(pc, esp); // get firmware version and set muxing of antenna
    bool ConnectedToWiFi = connectToWiFi(pc, esp, Specs);
    ...
    const int NumPorts = Specs.Ports.size();
